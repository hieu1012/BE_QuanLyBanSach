...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query("SELECT p FROM Product p WHERE p.price BETWEEN :minPrice AND :maxPrice ORDER BY p.createdAt DESC")
    ...
    @Query("SELECT p FROM Product p WHERE p.category.id = :categoryId AND p.price BETWEEN :minPrice AND :maxPrice ORDER BY p.createdAt DESC")
    ...
    @Query("SELECT p FROM Product p WHERE p.stock > :minStock ORDER BY p.createdAt DESC")
    Page<Product> findByStockGreaterThan(@Param("minStock") int minStock, Pageable pageable);

    @Query("SELECT p FROM Product p WHERE p.stock <= :maxStock ORDER BY p.createdAt DESC")
    ...
    @Query("SELECT p FROM Product p WHERE p.category.id = :categoryId AND p.stock > :minStock ORDER BY p.createdAt DESC")
    ...
    @Query("SELECT p FROM Product p WHERE p.price BETWEEN :minPrice AND :maxPrice AND p.stock > :minStock ORDER BY p.createdAt DESC")
